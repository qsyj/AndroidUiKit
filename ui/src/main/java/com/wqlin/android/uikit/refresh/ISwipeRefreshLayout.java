...
public class ISwipeRefreshLayout extends ViewGroup implements NestedScrollingParent,
    ...
    /**
     * NestedScrolling是否正在进行
     */
    boolean isNestedScrolling = false;

    /**
     * 在onNestedPreScroll()  onNestedScroll()  onStopNestedScroll()等方法是否需要从新 获取mTotalUnconsumed的值(根据mRefreshView.getTranslationY()值计算)  true时消费掉Y值 但不滑动
     */
    boolean isNestedAfreshTotalUnconsumed = false;
    /**
     * 是否刷新完成
     */
    boolean isRefreshComplete = false;
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationStart(Animation animation) {
            isAniming = true;
            getAnimHanlder().removeMessages(MESSAGE_WHAT_ANIM_END);
            getAnimHanlder().sendEmptyMessageDelayed(MESSAGE_WHAT_ANIM_END, animation.getDuration());
        }
...
//                        System.out.println(" onRefresh !!!");
            ...
            if (mRefreshView.getTranslationY() > 0) {
                animateOffsetToStartPosition(mRefreshListener);
            } else {
                reset();
            }
    ...
    private AnimHandler getAnimHanlder() {
        if (mAnimHandler == null) {
            mAnimHandler = new AnimHandler(this);
        } else {
            WeakReference<ISwipeRefreshLayout> weak = mAnimHandler.getRefreshLayoutWeak();
            if (weak == null || weak.get() == null) {
                mAnimHandler.setRefreshLayoutWeak(this);
            }
        }
        return mAnimHandler;
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        reset();
        if (mAnimHandler != null) {
            mAnimHandler.removeMessages(MESSAGE_WHAT_ANIM_END);
        }
    }
...
//        System.out.println("setRefreshing refreshing "+refreshing);
        if(mRefreshView == null || !isEnabled()||!isRefreshEnabled()){
            return;
        }
        boolean notify;
        ...
        setRefreshing(refreshing, notify /* notify */);
...
//                listener.onAnimationEnd(null);
    ...
    private void setRefreshing(boolean refreshing, final boolean notify) {
        ...
        if (mRefreshing) {
            ...
            if (!isAniming) {
                if (mRefreshView.getTranslationY() > 0&&!isNestedScrolling) {
                    animateOffsetToStartPosition(mRefreshListener);
                } else {
                    if (mRefreshView.getTranslationY() <= 0) {
                        reset();
    ...
    private void updateBaseValues(int refreshViewHeight) {
        ...
        setDistanceToTriggerSync(Math.round(mRefreshViewHeight * 3f));
    }
...
//        float translationY = mRefreshView.getTranslationY();
        boolean b = false;
        if (!isRefreshEnabled()) {
            b=startNestedScroll(nestedScrollAxes & ViewCompat.SCROLL_AXIS_VERTICAL);
        } else {
            b = isEnabled() && !mReturningToStart && !isAniming /*&& translationY > 0*/
    ...
    public void onStopNestedScroll(View target) {
        if (isRefreshEnabled()) {
            ...
            if (mTotalUnconsumed > 0) {
                if (!isAniming && !isNestedAfreshTotalUnconsumed) {
    ...
    public void onNestedScroll(final View target, final int dxConsumed, final int dyConsumed,
        ...
        if (dy < 0 && !canChildScrollUp()) {
            if (mTotalUnconsumed == 0) {
                float translationY = mRefreshView.getTranslationY();
                if ( translationY> 0) {
                    mTotalUnconsumed =translationY/DRAG_RATE;
                }
            }
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        float progress = overscrollTop / mRefreshViewHeight;
        if (!isRefreshing()&&!isRefreshComplete) {
...
//        System.out.println(" finishSpinner overescrollTop " + overscrollTop + "; mTotalDragDistance = " + mTotalDragDistance);
        if (overscrollTop >= mRefreshViewHeight) {
            if (!mRefreshing) {
                if (isRefreshComplete) {//刷新已经完成
                    animateOffsetToStartPosition(mRefreshListener);
                } else {
    ...
    private boolean canNotPullToRefresh() {
        return !isEnabled() || mReturningToStart || canChildScrollUp()
                || mRefreshing || mNestedScrollInProgress || mRefreshView == null||!isRefreshEnabled();
    }
...
//            mProgress.setAlpha(STARTING_PROGRESS_ALPHA);
    ...
    /**
     * 处理动画消息  onAnimationEnd()有bug可能不执行
     */
    static class AnimHandler extends Handler {
        private WeakReference<ISwipeRefreshLayout> mRefreshLayoutWeak;
        public AnimHandler(ISwipeRefreshLayout iSwipeRefreshLayout) {
            mRefreshLayoutWeak = new WeakReference<ISwipeRefreshLayout>(iSwipeRefreshLayout);
        }

        public WeakReference<ISwipeRefreshLayout> getRefreshLayoutWeak() {
            return mRefreshLayoutWeak;
        }

        public void setRefreshLayoutWeak(ISwipeRefreshLayout iSwipeRefreshLayout) {
            mRefreshLayoutWeak = new WeakReference<ISwipeRefreshLayout>(iSwipeRefreshLayout);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                case MESSAGE_WHAT_ANIM_END:
                    if (mRefreshLayoutWeak != null&&mRefreshLayoutWeak.get()!=null) {
                        mRefreshLayoutWeak.get().onRefreshAnimationEnd();
                    }
                    break;
            }
