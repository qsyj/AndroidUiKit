 ...
 */
public class StrongTabLayout extends HorizontalScrollView{
    ...
    /**
     * Gravity used to fill the {@link StrongTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link StrongTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    private StrongTabLayoutOnPageChangeListener mPageChangeListener;
    ...
    public StrongTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray sta = context.obtainStyledAttributes(attrs, R.styleable.StrongTabLayout);
        if (sta != null) {
            try {
                setSelectedTabIndicatorWidth(sta.getDimensionPixelSize(R.styleable.StrongTabLayout_selectedIndicatorWidth,-1));
            } finally {
                sta.recycle();
            }
        }
     ...
     *
     * @attr ref R.styleable#StrongTabLayout_tabIndicatorColor
     */
     ...
     *
     * @attr ref R.styleable#StrongTabLayout_tabIndicatorHeight
     */
     ...
     * 点击tab切换页面 是否允许指示器动画
     * */
    public void setAnimIndicatorWhenClickTab(boolean animIndicatorWhenClickTab) {
        mIsAnimIndicatorWhenClickTab = animIndicatorWhenClickTab;
    }
    ...
    public void addTab(@NonNull Tab tab, int position, boolean setSelected) {
        if (tab.mParent != this) {
            throw new IllegalArgumentException("Tab belongs to a different StrongTabLayout.");
        }
    ...
    /**
     * Add a {@link StrongTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     *
    ...
    /**
     * Remove the given {@link StrongTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link StrongTabLayout.OnTabSelectedListener}s.
     */
    ...
    public void removeTab(Tab tab) {
        if (tab.mParent != this) {
            throw new IllegalArgumentException("Tab does not belong to this StrongTabLayout.");
        }
     ...
     *
     * @attr ref R.styleable#StrongTabLayout_tabMode
     */
    ...
    /**
     * Returns the current mode used by this {@link StrongTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
     ...
     *
     * @attr ref R.styleable#StrongTabLayout_tabGravity
     */
     ...
     * Sets the text colors for the different states (normal, selected) used for the tabs.
     *
     * @attr ref R.styleable#StrongTabLayout_tabTextColor
    ...
    /**
     * The one-stop shop for setting up this {@link StrongTabLayout} with a {@link ViewPager}.
     *
     ...
     * The one-stop shop for setting up this {@link StrongTabLayout} with a {@link ViewPager}.
     *
     * <p>This method will link the given ViewPager and this StrongTabLayout together so that
    ...
    private void setupWithViewPager(@Nullable final ViewPager viewPager, boolean autoRefresh,
        ...
        if (viewPager != null) {
            ...
            if (mPageChangeListener == null) {
                mPageChangeListener = new StrongTabLayoutOnPageChangeListener(this);
            }
    ...
    /**
     * @deprecated Use {@link #setupWithViewPager(ViewPager)} to link a StrongTabLayout with a ViewPager
     *             together. When that method is used, the StrongTabLayout will be automatically updated
    ...
    private void addViewInternal(final View child) {
        if (child instanceof TabItem) {
            ...
            throw new IllegalArgumentException("Only TabItem instances can be added to StrongTabLayout");
        }
    }
    ...
    public static final class Tab {
        ...
        public Tab setIcon(@DrawableRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a StrongTabLayout");
            }
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        void setSelectedIndicatorWidth(int width) {
            if (mSelectedIndicatorHeight != width) {
        ...
        private void setIndicatorPosition(int position) {
            View view = getChildAt(position);
            if (view != null) {
                setIndicatorPosition(view.getLeft(), view.getRight());
            }
        }
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                float left = mIndicatorLeft;
                float right = mIndicatorRight;
                float width = right - left;
                if (mSelectedIndicatorWidth > 0&&width>mSelectedIndicatorWidth) {
                    float dd = (width - mSelectedIndicatorWidth) / 2f;
                    left += dd;
                    right -= dd;
                }
     ...
     * necessary calls back to the provided {@link StrongTabLayout} so that the tab position is
     * kept in sync.
     *
     * <p>This class stores the provided StrongTabLayout weakly, meaning that you can use
     * {@link ViewPager#addOnPageChangeListener(ViewPager.OnPageChangeListener)
     * addOnPageChangeListener(OnPageChangeListener)} without removing the listener and
     * not cause a leak.
     */
    public static class StrongTabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<StrongTabLayout> mStrongTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;
        public StrongTabLayoutOnPageChangeListener(StrongTabLayout StrongTabLayout) {
            mStrongTabLayoutRef = new WeakReference<>(StrongTabLayout);
        }
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final StrongTabLayout StrongTabLayout = mStrongTabLayoutRef.get();
            if (StrongTabLayout != null) {
                ...
                if (!StrongTabLayout.isAnimateToTab)
                    StrongTabLayout.setScrollPosition(position, positionOffset, updateText, updateIndicator);
            }
        }
        ...
        public void onPageSelected(final int position) {
            final StrongTabLayout StrongTabLayout = mStrongTabLayoutRef.get();
            if (StrongTabLayout != null && StrongTabLayout.getSelectedTabPosition() != position
                    && position < StrongTabLayout.getTabCount()) {
                ...
                final boolean updateIndicator = mScrollState == SCROLL_STATE_IDLE
                        ...
                        && mPreviousScrollState == SCROLL_STATE_IDLE);
                if (!StrongTabLayout.isAnimateToTab)
                    StrongTabLayout.selectTab(StrongTabLayout.getTabAt(position), updateIndicator);
            }
        }
    ...
    /**
     * A {@link StrongTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements StrongTabLayout.OnTabSelectedListener {
        ...
        @Override
        public void onTabSelected(StrongTabLayout.Tab tab) {
            mViewPager.setCurrentItem(tab.getPosition());
        }

        @Override
        public void onTabUnselected(StrongTabLayout.Tab tab) {
            // No-op
        }

        @Override
        public void onTabReselected(StrongTabLayout.Tab tab) {
            // No-op
        }
    }
